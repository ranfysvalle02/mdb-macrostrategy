"""Unit tests for the basic-angle harness (basic.bench.common).

These never touch a database, so they run anywhere (including the CI lint+unit
job) in milliseconds. Engine-agnostic statistics are covered separately in
``shared/tests/test_stats.py``.
"""
